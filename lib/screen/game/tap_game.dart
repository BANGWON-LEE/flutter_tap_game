...
class _TapGameState extends State<TapGame> {
  ...
  void setEditRecordHttp() async {
    ...
    prefs.setInt('recent_rc_record', _time);
    // print('prevR_1  ${prevRecord}');
    // print('_time_1 ${_time}');
    if (prevRecord != null && (prevRecord! > _time || prevRecord == 0)) {
                                              ...
                                              child: SizedBox(
                                                ...
                                                child: ElevatedButton(
                                                    style: ElevatedButton.styleFrom(
                                                        primary: cnt == 0
                                                            ? Colors.blue
                                                            : (random - cnt) % int.parse(num.value) == 1
                                                                ? Color(codeHex)
                                                                : (random + cnt) % int.parse(num.value) == 3
                                                                    ? Color(secondCodeHex)
                                                                    : (random + cnt) % int.parse(num.value) == 2
                                                                        ? Color(thirdCodeHex)
                                                                        : ((random + 31) / int.parse(num.value)) % 2 == 0
                                                                            ? Color(fourthCodeHex)
                                                                            : Colors.pinkAccent),
                                                    onPressed: () {
                                                      cntStr = cnt.toString();
                                                      hexNum =
                                                          '0xffC${kind[indexRandom + 3]}5C5C';
                                                      secondHexNum =
                                                          '0xffF7${kind[indexRandom + 1]}F50';
                                                      thirdHexNum =
                                                          '0xff2F${kind[indexRandom]}Df4';
                                                      fourthHexNum =
                                                          '0xff2F${kind[indexRandom + 2]}Df4';

                                                      codeHex =
                                                          int.parse(hexNum);
                                                      secondCodeHex = int.parse(
                                                          secondHexNum);
                                                      thirdCodeHex = int.parse(
                                                          thirdHexNum);
                                                      fourthCodeHex = int.parse(
                                                          fourthHexNum);
              ...
              child: startSignal == true
                  ...
                  : ElevatedButton(
                      onPressed: () {
                        ...
                        List mix =
                            btnCount.reduce((acr, cur) => [...acr, ...cur]);
                        mix.shuffle();
...
class _TimeCheckState extends State<TimeCheck> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Container(
          child: widget.signal == false
              ? null
              : Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: widget.gameCount
                      .map(
                        (num) => Container(
